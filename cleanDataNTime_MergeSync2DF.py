...
for daily in range(len(days)):
    ...
    i = 0
    try:
        t0 = datetime.strptime(str(df.loc[i][0])[:10] + " 00:00:00",
                               "%Y-%m-%d %H:%M:%S")  # Time refrence for date conversion
    except:
        try:
            t0 = datetime.strptime(str(df.loc[i][0])[:10] + " 00:00:00", "%Y:%m:%d %H:%M:%S")
        except:
            try:
    ...
    # print("00:00:00 "+str(df2.loc[i][0])[:10])
    try:
        t0 = datetime.strptime(str(df2.loc[i][0])[:10] + " 00:00:00",
                               "%Y-%m-%d %H:%M:%S")  # Time refrence for date conversion
    except:
        try:
